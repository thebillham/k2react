...
class Vehicles extends React.Component {
  ...
  filterVehicle = vehicle => {
    ...
    if (
      ...
      this.state.officeFilters[vehicle.location]
    )
      filter = true;
    if (this.props.search) {
      if (
        !(vehicle.number + vehicle.location + vehicle.makemodel)
          .toLowerCase()
    ...
    if (this.state.dueFilterOn) {
      ...
      if (this.state.dueFilters["Requires Attention"]) {
        if (
          (check <= today || vehicle.roaduserkms <= vehicle.mileage) &&
          filter
        )
          filter = true;
        else if (
          ...
          reg > today &&
          service > today &&
          vehicle.servicekms > vehicle.mileage
        )
          filter = false;
      }
      if (
        this.state.dueFilters["WOF or Rego Expired"] &&
        wof > today &&
        reg > today
      )
        filter = false;
      if (
        this.state.dueFilters["Service Due"] &&
        service > today &&
        vehicle.servicekms > vehicle.mileage
      )
        filter = false;
      if (this.state.dueFilters["Checks Due"] && check > today) filter = false;
      if (this.state.dueFilters["Checks Due"] && !vehicle.lastcheck)
        filter = false;
      if (
        this.state.dueFilters["Road User Charges Due"] &&
        vehicle.roaduserkms > vehicle.mileage
      )
        filter = false;
      if (
        this.state.dueFilters["Road User Charges Due"] &&
                ...
                return (
                  <div key={chip} style={{ padding: 5 }}>
                    <Chip
                      ...
                      variant="outlined"
                      color={
                        this.state.dueFilters[chip] ? "secondary" : "default"
                      }
                      onClick={() => this.filterDue(chip)}
                      label={chip}
                    />
                  </div>
                );
          ...
          <div style={{ position: "relative", width: "70vw" }}>
              ...
              <Button
                ...
                onClick={() => {
                  this.props.showModal({
                    modalType: VEHICLE,
                    modalProps: { title: "Add New Vehicle" }
                  });
                }}
              >
                Add New Vehicle
            ...
            <div>
              {vehicles
                ...
                .map(vehicle => {
                  return <VehicleList key={vehicle.number} vehicle={vehicle} />;
                })}
            </div>
          </div>
        </Paper>
