...
class CocList extends React.Component {
  ...
  printLabReport = version => {
    ...
    if (report.version && report.version > 1) {
      ...
      [...Array(report.version - 1).keys()].forEach(i => {
        let formatDate = (this.props.job.versionHistory[i+1].issueDate instanceof Date) ? this.props.job.versionHistory[i+1].issueDate : this.props.job.versionHistory[i+1].issueDate.toDate()
  ...
  printCoc = job => {
    ...
    let url = 'http://api.k2.co.nz/v1/doc/scripts/asbestos/lab/coc.php?report=' + JSON.stringify(report);
    console.log(url);
    this.setState({ url: url });
    window.open(url);
    // fetch('http://api.k2.co.nz/v1/doc/scripts/asbestos/issue/labreport_singlepage.php?report=' + JSON.stringify(report));
        ...
        <ExpansionPanelDetails>
          <List>
            <div>
              ...
              <Button style={{ marginLeft: 5, }} variant='outlined' disabled={ job.versionUpToDate } onClick={() => {
                ...
                if (!allSamplesReported && !window.confirm("Not all samples in the chain of custody have been checked off. These will not be included in this issue. Proceed with issuing?")) return;
                if (job.currentVersion) {
                  this.props.showModal({ modalType: UPDATECERTIFICATEVERSION, modalProps: { doc: { changes: '', version: version, }, issueLabReport: this.issueLabReport} });
                } else {
          ...
          { samples[job.uid] && Object.values(samples[job.uid]).map(sample => {
            ...
            menu[sample.samplenumber.toString()] = (
              <Menu
                id={`${job.jobNumber}-${sample.samplenumber.toString()}`}
                anchorEl={ this.state.sampleAnchorEl }
                open={Boolean(this.state.sampleAnchorEl)}
                onClose={() => { this.setState({ sampleAnchorEl: null }) }}
                style={{ padding: 0, }}
                >
                ...
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '70vw'}}>
                  ...
                  <div style={{ width: '40vw', display: 'flex', flexDirection: 'row',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'row', }}>
                      <div style={{ backgroundColor: chdivcolor, borderRadius: 5, }}>
                        <Button variant='outlined' disabled={ !sample.receivedbylab } style={{ margin: 5, color: chcolor, }} onClick={ () => { this.toggleResult(sample, 'ch') }}>CH</Button>
                      </div>
                      <div style={{ backgroundColor: amdivcolor, borderRadius: 5, }}>
                        <Button variant='outlined' disabled={ !sample.receivedbylab } style={{ margin: 5, color: amcolor, }} onClick={ () => { this.toggleResult(sample, 'am') }}>AM</Button>
                      </div>
                      <div style={{ backgroundColor: crdivcolor, borderRadius: 5, }}>
                        <Button variant='outlined' disabled={ !sample.receivedbylab } style={{ margin: 5, color: crcolor, }} onClick={ () => { this.toggleResult(sample, 'cr') }}>CR</Button>
                      </div>
                      <div style={{ backgroundColor: umfdivcolor, borderRadius: 5, }}>
                        <Button variant='outlined' disabled={ !sample.receivedbylab } style={{ margin: 5, color: umfcolor, }} onClick={ () => { this.toggleResult(sample, 'umf') }}>UMF</Button>
                      </div>
                    </div>
                    <div style={{ width: 30 }} />
                    <div style={{ backgroundColor: nodivcolor, borderRadius: 5 }}>
                      <Button variant='outlined' disabled={ !sample.receivedbylab } style={{ margin: 5, color: nocolor, }} onClick={ () => { this.toggleResult(sample, 'no') }}>NO</Button>
                    </div>
                    ...
                    <Menu
                      ...
                      open={Boolean(this.state.sampleAnchorEl[sample.samplenumber])}
                      onClose={() => { this.setState({ sampleAnchorEl: { [sample.samplenumber]: null } }) }}
                      style={{ padding: 0, }}
                      >
                      <MenuItem key={`${job.jobNumber}-${sample.samplenumber.toString()}-WA`} onClick={() => {
