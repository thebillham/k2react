...
function VehicleList(props) {
  ...
  return (
    <ListItem dense className={classes.hoverItem} key={vehicle.number}>
      <Grid
        container
        direction="row"
        justify="flex-start"
      ...
      >
        <Grid item xs={2} style={{ marginTop: 32 }}>
          ...
          <IconButton
            onClick={() => {
              props.showModal({
                modalType: VEHICLE,
                modalProps: { doc: vehicle, title: "Edit Vehicle" }
              });
            }}
          >
            <Edit />
        ...
        <Grid item xs={4} style={{ fontWeight: 100, fontSize: 14 }}>
          ...
          <div>
            <i>{vehicle.makemodel + " (" + vehicle.year + ")"}</i>
          </div>
          {vehicle.wof && (
            <div>
              <span style={{ fontWeight: 450 }}>WOF Expiry:</span>{" "}
              <span style={{ color: wofcolor }}>
                <FormattedDate
                  value={vehicle.wof}
                  month="long"
                  day="numeric"
          ...
          {vehicle.reg && (
            <div>
              <span style={{ fontWeight: 450 }}>Reg Expiry:</span>{" "}
        ...
        <Grid
          item
          xs={3}
          style={{ marginTop: 16, fontWeight: 100, fontSize: 14 }}
        >
          {vehicle.lastservice && (
            <div>
              <span style={{ fontWeight: 450 }}>Last Service:</span>{" "}
          ...
          {vehicle.lastcheck && (
            <div>
              <span style={{ fontWeight: 450 }}>Last Check:</span>{" "}
          ...
          {vehicle.mileage && (
            <div>
              <span style={{ fontWeight: 450 }}>Mileage:</span>{" "}
              <span>{vehicle.mileage + " kms"}</span>
            </div>
          )}
          {vehicle.servicekms && (
            <div>
              <span style={{ fontWeight: 450 }}>Next Service At:</span>{" "}
              <span style={{ color: servicekmcolor }}>
                {vehicle.servicekms + " kms"}
              </span>
            </div>
          )}
          {vehicle.roaduserkms && (
            <div>
              <span style={{ fontWeight: 450 }}>Road User Charges To:</span>{" "}
              <span style={{ color: roadusercolor }}>
                {vehicle.roaduserkms + " kms"}
              </span>
            </div>
          )}
        </Grid>
        ...
        <Grid item xs={1} style={{ marginTop: 32 }}>
          <IconButton
            onClick={() => {
              if (
                window.confirm("Are you sure you wish to delete this vehicle?")
              )
