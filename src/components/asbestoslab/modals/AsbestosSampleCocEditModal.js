...
import { handleSampleChange, writeDescription, getAirSampleData, updateResultMap, getSampleColors, getBasicResult, } from '../../../actions/asbestosLab';
...
class AsbestosSampleCocEditModal extends React.PureComponent {
  ...
  swapSample = onComplete => {
    ...
    let log = {
      type: 'ID Change',
      log: `Samples ${sample.jobNumber}-${sample.sampleNumber} (${writeDescription(sample)}) and ${sample.jobNumber}-${this.state.sampleSwap} (${writeDescription(samples[this.state.sampleSwap])}) swapped numbers.`,
      chainOfCustody: sample.cocUid,
      sample: sample.uid,
    };
    addLog("asbestosLab", log, me);
  ...
  saveSample = (onComplete) => {
    ...
    if (this.state.sampleDelete && window.confirm("Are you sure you wish to delete this sample?")) {
      ...
      } else if (samples[this.state.sampleSwap] === undefined && window.confirm(`Are you sure you wish to move this sample to number ${this.state.sampleSwap}`)) {
        // Move to sample number
        this.moveSample(onComplete);
      } else if (samples[this.state.sampleSwap] !== undefined && window.confirm(`There is already a sample using that sample number. Do you wish to swap sample ${this.state.sample.sampleNumber} with sample ${this.state.sampleSwap} (${writeDescription(samples[this.state.sampleSwap])})?`)) {
        // Swap sample number
        ...
        {sample && (
          <div>
            {sample.sampleType === 'air' ?
              <div>
                <SuggestionField that={this} label='Location' suggestions='airLocationSuggestions'
                ...
                <div className={classes.flexRow}>
                  ...
                  />
                  {sample.initialFlowRate && sample.finalFlowRate ? <div className={calcs.differenceTooHigh ? classes.informationBoxError : (calcs.sampleRateLow || calcs.sampleRateHigh) ? classes.informationBoxWarning : classes.informationBoxOk}>
                ...
                <div className={classes.flexRowBottom}>
                  ...
                  />
                  {calcs.sampleVolume ? <div className={calcs.sampleVolumeMuchTooLow ? classes.informationBoxError : calcs.sampleVolumeTooLow ? classes.informationBoxWarning : classes.informationBoxOk}>
                ...
                {modalProps.doc.historicalCoc &&
                  <div className={classes.flexRow}>
                    ...
                    />
                    {calcs.reportConcentration ? <div className={calcs.reportConcentration.includes("<") ? classes.informationBoxOk : classes.informationBoxError}>
                ...
                {modalProps.doc.historicalCoc &&
                  <div className={classNames(classes.flexRow, classes.columnExtraLarge)}>
                    {['ch','am','cr','umf','no'].map(res => {
                      return AsbButton(classes[`colorsButton${colors[res]}`], classes[`colorsDiv${colors[res]}`], res, () => this.handleResultClick(res))
                    })}
                  </div>
                }
              </div>
